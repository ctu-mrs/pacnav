...
'
  'NodeCrasher' 'waitForRos; roslaunch mrs_uav_general node_crash_checker.launch
'
  'Status' 'waitForRos; roslaunch mrs_uav_status status.launch
'
  'Control' 'waitForRos; roslaunch mrs_uav_general core.launch config_odometry:=./custom_configs/odometry.yaml config_constraint_manager:=./custom_configs/constraint_manager.yaml config_uav_manager:=./custom_configs/uav_manager.yaml config_gain_manager:=./custom_configs/gain_manager.yaml config_control_manager:=./custom_configs/control_manager.yaml config_landoff_tracker:=./custom_configs/landoff_tracker.yaml
'
  'AutoStart' 'waitForRos; roslaunch mrs_uav_general automatic_start.launch custom_config:=./custom_configs/automatic_start.yaml
'

  'Vision' 'waitForRos; roslaunch openvino_ros uav.launch
'
  'KF' 'waitForRos; roslaunch pose_filtering uav.launch
'
...
'
  'roscore' 'roscore
'
  'swarm_controller' 'waitForControl; roslaunch swarm_control_manager swarm_control_manager.launch controller_pkg:=pacnav origin_frame:=local_origin uav_frame:=fcu_untilted
'
  'run_swarm' 'history -s rosservice call /'"$UAV_NAME"'/uav_manager/land;
...
'
  'goto' 'history -s rosservice call /'"$UAV_NAME"'/pacnav_controller/goto
'
  'shared_gps_aggr' 'waitForControl; roslaunch swarm_utils shared_gps_aggr.launch
'
)
